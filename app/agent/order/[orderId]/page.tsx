...
export default async function AgentOrderDetailPage({
  ...
  const owned = await getOwnershipCheck(orderId, agentId);
  if (!owned) notFound();
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
        <section className="flex flex-col gap-3">
          ...
          <AgentStatusActions orderId={order.id} status={order.status} />
        </section>
...
async function getOwnershipCheck(orderId: string, agentId: string) {
